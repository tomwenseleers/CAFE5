...
$r\sim\operatorname{Gamma}(\alpha,\text{rate}=\alpha)$. Numerical integration uses
...
The simple preset has $\pi(a)=\mathbf1\{a=1\}$. The Poisson option uses
...
$\log\Pr(A)=\log[-\operatorname{expm1}(\log\Pr(Y=0))]$. This avoids
...
$$
\widetilde L_{i,base}=\max_{a\geq1}\{\pi(a)F_i(a)\},\qquad
\widetilde L_{i,gamma}=\sum_k w_k\max_{a\geq1}\{\pi(a)F_{i,k}(a)\}.
$$
...
$$
p_i^{legacy}=\max_{a\in\mathcal R_i}
...
**same free parameters** on each dataset to obtain $\hat\theta_b^*$. Compute

$$
T_i=\log L_i^+(\hat\theta),\qquad
T_{bj}^*=\log L^+(Y_{bj}^*;\hat\theta_b^*).
$$

Within each replicate form the corrected empirical lower tail, then average:

$$
f_{bi}=\frac{1+\sum_{j=1}^n\mathbf1\{T_{bj}^*\leq T_i\}}{n+1},
\qquad \hat p_i=\frac1B\sum_{b=1}^B f_{bi}.
$$
...
+\tfrac12\sum_{m:P_{am}=P_{ab}}P_{am}.
$$
...
$$
u_{bie}=\frac{1+\sum_j\mathbf1\{D_{bje}^*\geq D_{ie}\}}{n+1},\qquad
l_{bie}=\frac{1+\sum_j\mathbf1\{D_{bje}^*\leq D_{ie}\}}{n+1},
$$

$$
\hat p_{ie}=\min\{1,\;2\min(\bar u_{ie},\bar l_{ie})\}.
$$
